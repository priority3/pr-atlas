...
[pr-atlas](https://github.com/priority3/pr-atlas) 的运行时。除 `@pr-atlas/schema` 外零依赖。

包含：

- `ConnectorRegistry` 与 `runConnector`（调用 `collect` 前强制校验 `config_schema`）
- `createManualCapture`：把一个 URL 或一段文本变成 capture
- `OutboxStore`：状态由文件位置表示的 outbox，`.atlas/outbox/{pending,sent,failed}`，状态流转是一次原子 rename
...
```ts
import { ConnectorRegistry, runConnector, OutboxStore, syncOutbox, createDeliverer } from '@pr-atlas/core'
...
const outbox = new OutboxStore('.atlas/outbox')
for (const capture of result.captures) await outbox.enqueue(capture)

const deliverer = createDeliverer('file', { directory: './atlas-export' }, 'local')
