...
[pr-atlas](https://github.com/priority3/pr-atlas) 的内置内容源。只依赖 `@pr-atlas/schema`，**不依赖 `@pr-atlas/core`** —— Connector 不知道 registry、outbox 或 CLI 的存在，因此可以脱离运行时单独测试。

```bash
npm i @pr-atlas/connectors
```
...
```ts
import { createGenericWebConnector } from '@pr-atlas/connectors/generic-web'
```
...
```ts
import { createPriorityMeBlogConnector } from '@pr-atlas/connectors/priority-me-blog'
```
